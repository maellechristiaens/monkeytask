...
rank = TrialRecord.User.pictures_left5(randi(length(TrialRecord.User.pictures_left5), 1)); %choose the number of the picture to show
...
scene_step5 = create_scene(all_three);
...
if 0==error_type || 4==error_type %if the subject clicks on a picture
    TrialRecord.User.pictures_left5(TrialRecord.User.pictures_left5 == rank) = []; %delete the element that has been choosen
    run_scene(scene_sound,100); %play the sound once
    idle(0);                 
    goodmonkey(100, 'juiceline',1, 'numreward',5, 'pausetime',0); 
    %and give the reward once
else
